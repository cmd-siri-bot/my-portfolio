# ============================================================
#  True fix: flexbox-based centering (not margin-based)
#    powershell -ExecutionPolicy Bypass -File .\setup-center3.ps1
#
#  Every section, the header, and the footer now use
#  "flex justify-center" on the outer wrapper, so the content
#  column is centered by flexbox rather than margin:auto.
#  This works even if something elsewhere was overriding margins.
# ============================================================
...
Write-Host ""
Write-Host "Done - centering now uses flexbox on every section/header/footer." -ForegroundColor Green
Write-Host "Close and restart 'npm run dev' fully (Ctrl+C then npm run dev again)," -ForegroundColor Yellow
Write-Host "not just a browser refresh, to be safe. Then hard-refresh the browser." -ForegroundColor Yellow
Write-Host ""
Write-Host "Then: git add . ; git commit -m 'Fix centering with flexbox' ; git push" -ForegroundColor Yellow
